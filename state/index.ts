export default class Index<State> {
    ...
    async dispatchAsync(event: string, state: Promise<Partial<State>> | ((prev: State) => Promise<Partial<State>>)) {
        if (state instanceof Function)
            this.setState(await state(this.globState));
        else
